...
// Catch-all route for SPA - MUST be last before error handler
if (NODE_ENV === 'production') {
  app.get('*', (req, res) => {
...
// Catch all undefined API routes (before SPA catch-all)
app.use('/api/*', (req, res) => {
  console.log(`❓ Unknown API route called: ${req.method} ${req.originalUrl}`);
  res.status(404).json({
    success: false,
