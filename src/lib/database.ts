...
// Tipos para respuestas de base de datos
...
// Query Builder optimizado para Neon
export class NeonQueryBuilder {
  ...
  private operation?: 'insert' | 'update' | 'delete' | 'select';
  ...
  private buildWhereClause(values: any[], paramCounter: { count: number }): string {
    ...
    const clauses = this.whereConditions.map(condition => {
      if (condition.operator === 'IN') {
        const placeholders = condition.value.map(() => `$${++paramCounter.count}`);
...
// Cliente principal de base de datos
export class NeonDatabaseClient {
  from(table: string) {
    return new NeonQueryBuilder(table);
  }

  async query(text: string, params?: any[]): Promise<DatabaseResponse> {
    try {
